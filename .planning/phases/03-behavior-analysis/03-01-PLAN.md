...
depends_on: []
files_modified:
  - src/mod_sentinel/llm/__init__.py
...
must_haves:
  ...
  artifacts:
    - path: "src/mod_sentinel/agents/behavior_agent.py"
      provides: "Behavior Agent"
    - path: "src/mod_sentinel/llm/client.py"
      provides: "LLM client abstraction"
  key_links:
    - from: "src/mod_sentinel/agents/behavior_agent.py"
      to: "src/mod_sentinel/llm/client.py"
...
Do not hardcode provider-specific assumptions in the agent logic; keep them in `llm/client.py`.
  ...
  <verify>python3 -c "from mod_sentinel.llm.client import StubLLMClient; print(StubLLMClient().complete_json('s','u'))"</verify>
