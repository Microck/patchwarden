...
must_haves:
  ...
  artifacts:
    - path: "src/mod_sentinel/pipeline/snippet_select.py"
      provides: "Snippet selection for prompts"
  key_links:
    - from: "src/mod_sentinel/api/routes/scan.py"
      to: "src/mod_sentinel/agents/behavior_agent.py"
...
<task type="auto">
  ...
  <files>src/mod_sentinel/pipeline/snippet_select.py</files>
  <action>
...
This is to keep LLM prompts stable and fast.
  ...
  <verify>python3 -c "from mod_sentinel.pipeline.snippet_select import select_snippets; print('ok')"</verify>
