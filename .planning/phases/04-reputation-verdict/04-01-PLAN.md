...
must_haves:
  ...
  artifacts:
    - path: "src/jarspect/agents/reputation_agent.py"
      provides: "Reputation Agent"
    - path: "src/jarspect/store/reputation_fixtures.py"
      provides: "File-backed author history + community report fixtures"
  key_links:
    - from: "src/jarspect/api/routes/scan.py"
      to: "src/jarspect/agents/reputation_agent.py"
...
Store fixtures as JSON files under `src/jarspect/fixtures/reputation/` and load them via a small helper (`store/reputation_fixtures.py`). Keep it deterministic and local-only.
