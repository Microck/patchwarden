...
must_haves:
  ...
  artifacts:
    - path: "src/mod_sentinel/agents/verdict_agent.py"
      provides: "Verdict Agent"
    - path: "src/mod_sentinel/models/verdict.py"
      provides: "Verdict models"
  key_links:
    - from: "src/mod_sentinel/agents/verdict_agent.py"
      to: "src/mod_sentinel/models/verdict.py"
...
<task type="auto">
  ...
  <files>src/mod_sentinel/models/verdict.py</files>
  <action>
...
This must support "specific suspicious indicators listed" (VERD-04).
  ...
  <verify>python3 -c "from mod_sentinel.models.verdict import Verdict"</verify>
