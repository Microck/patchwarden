...
files_modified:
  ...
  - src/jarspect/store/__init__.py
...
must_haves:
  ...
  artifacts:
    - path: "src/jarspect/pipeline/scan_pipeline.py"
      provides: "Orchestrated analysis pipeline"
    - path: "src/jarspect/store/scans.py"
      provides: "Scan result persistence"
  key_links:
    - from: "src/jarspect/api/routes/scans.py"
      to: "src/jarspect/store/scans.py"
      via: "load by id"
      pattern: "get_scan"
---
...
Return a `scan_id` (uuid) for every scan.
  ...
  <verify>python3 -c "from jarspect.pipeline.scan_pipeline import run_scan; print('ok')"</verify>
