...
files_modified:
  ...
  - src/mod_sentinel/analysis/__init__.py
...
must_haves:
  ...
  artifacts:
    - path: "src/mod_sentinel/agents/intake_agent.py"
      provides: "Intake agent"
    - path: "src/mod_sentinel/analysis/jar_extract.py"
      provides: "Safe jar extraction + inspection"
  key_links:
    - from: "src/mod_sentinel/api/routes/scan.py"
      to: "src/mod_sentinel/agents/intake_agent.py"
...
Define Pydantic models in `src/mod_sentinel/models/intake.py` for `IntakeResult` including:
- `upload_id`, `filename`
