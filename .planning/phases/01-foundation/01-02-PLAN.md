...
files_modified:
  - .env.example
  - src/mod_sentinel/storage/__init__.py
...
must_haves:
  ...
  artifacts:
    - path: "src/mod_sentinel/api/routes/upload.py"
      provides: "Upload endpoint"
    - path: "src/mod_sentinel/storage/base.py"
      provides: "Storage backend interface"
  key_links:
    - from: "src/mod_sentinel/api/routes/upload.py"
      to: "src/mod_sentinel/storage/base.py"
...
Do not require Azure credentials for local mode.
  ...
  <verify>python3 -c "from mod_sentinel.storage.local import LocalStorage; LocalStorage('.local-data/uploads')"</verify>
...
src/mod_sentinel/storage/azure_blob.py
  ...
  <action>
Implement `AzureBlobStorage` in `src/mod_sentinel/storage/azure_blob.py` behind env gating:
...
Keep the rest of the codebase depending only on the storage interface; no Azure imports should be required when using local mode.
  ...
  <verify>python3 -c "import mod_sentinel.storage.azure_blob as m; print('imported')"</verify>
...
Wire the router into `src/mod_sentinel/api/main.py`.
