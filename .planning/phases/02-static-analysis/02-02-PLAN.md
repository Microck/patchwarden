...
must_haves:
  ...
  artifacts:
    - path: "src/jarspect/analysis/patterns.py"
      provides: "Suspicious pattern catalog"
    - path: "src/jarspect/analysis/static_scan.py"
      provides: "Pattern matching engine"
  key_links:
    - from: "src/jarspect/analysis/static_scan.py"
      to: "src/jarspect/analysis/patterns.py"
...
<task type="auto">
  ...
  <files>src/jarspect/analysis/patterns.py</files>
  <action>
...
Keep patterns benign: they are detection rules, not malware payloads.
  ...
  <verify>python3 -c "from jarspect.analysis.patterns import PATTERNS; assert len(PATTERNS) >= 10"</verify>
