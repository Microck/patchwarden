...
must_haves:
  ...
  artifacts:
    - path: "src/mod_sentinel/agents/static_agent.py"
      provides: "Static Agent implementation"
    - path: "src/mod_sentinel/api/routes/scan.py"
      provides: "Scan endpoint extended with static results"
  key_links:
    - from: "src/mod_sentinel/agents/static_agent.py"
      to: "src/mod_sentinel/analysis/static_scan.py"
...
<task type="auto">
  ...
  <files>src/mod_sentinel/agents/static_agent.py</files>
  <action>
...
Do not execute any mod code.
  ...
  <verify>python3 -c "from mod_sentinel.agents.static_agent import StaticAgent; print('ok')"</verify>
