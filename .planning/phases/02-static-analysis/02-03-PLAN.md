...
files_modified:
  ...
  - src/mod_sentinel/signatures/__init__.py
...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/mod_sentinel/signatures/store.py"
      provides: "Signature store interface"
  key_links:
    - from: "src/mod_sentinel/signatures/local_json.py"
...
The script should be safe to run multiple times (idempotent by signature id).
  ...
  <verify>python3 -c "from mod_sentinel.signatures.local_json import LocalJsonSignatureStore; LocalJsonSignatureStore()"</verify>
